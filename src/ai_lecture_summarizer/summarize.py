...
from .local_model import generate_summary_with_local_model
...
def summarize_transcript(
	...
	"""Summarize a transcript using LoRA or Gemini per SUMMARIZER_MODE env.

	Modes:
	- 'lora': require local adapter; raise if unavailable
	- 'gemini': force Gemini
	- anything else or unset: try LoRA, else fallback to Gemini
	"""
	mode = (os.getenv("SUMMARIZER_MODE") or "auto").lower()

	if mode == "lora":
		local = generate_summary_with_local_model(
			transcript_text=transcript_text,
			title_hint=title_hint,
			system_prompt=SYSTEM_PROMPT,
		)
		if not local:
			raise RuntimeError("Requested LoRA mode but no local adapter is available. Set LORA_ADAPTER_DIR or train an adapter.")
		return local.strip()
